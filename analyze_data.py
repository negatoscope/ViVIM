...
# Neutral points (for intensity) and ground truth (for specificity)
REFERENCE_LEVELS = {
    'brightness': 11,
    'contrast': 11,
    'saturation': 15,
...
def analyze_vim_data(url):
    ...
    # --- Remap old clarity scale (1-10) to new scale (1-21) ---
    # The clarity parameter was remapped from 10 to 21 levels mid-pilot.
    # Participants whose max clarity response is <= 10 used the old scale.
    clarity_rows = df_clean[df_clean['parameter'] == 'clarity']
    old_scale_participants = clarity_rows.groupby('sessionID')['selected_level'].max()
    old_scale_ids = old_scale_participants[old_scale_participants <= 10].index

    old_clarity_mask = (df_clean['parameter'] == 'clarity') & (df_clean['sessionID'].isin(old_scale_ids))
    n_remapped = old_clarity_mask.sum()
    df_clean.loc[old_clarity_mask, 'selected_level'] = 1 + (df_clean.loc[old_clarity_mask, 'selected_level'] - 1) * (20 / 9)
    print(f"\nRemapped {n_remapped} clarity responses from {len(old_scale_ids)} participants (old 1-10 scale -> new 1-21 scale).")
    ...
    # All three specificity params are direct-scored (higher level = higher fidelity)
    for param in SPECIFICITY_PARAMS:
        df_clean.loc[df_clean['parameter'] == param, 'specificity_fidelity'] = (df_clean['selected_level'] - 1) / 20 * 100
    ...
    print("(Higher = more intense/more specific)\n")
